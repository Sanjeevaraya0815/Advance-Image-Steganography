...
#         for y in range(height):
#             r, g, b = image.getpixel((x, y))
#             binary_data += to_binary(r)[-2:] + to_binary(g)[-2:] + to_binary(b)[-2:]

#             if len(binary_data) >= 24:
#                 try:
#                     all_bytes = [binary_data[i : i + 8] for i in range(0, len(binary_data), 8)]
#                     decoded_text = "".join([chr(int(byte, 2)) for byte in all_bytes if len(byte) == 8])
#                     end_marker = decoded_text.find("$$$")
#                     if end_marker != -1:
...
#     for _, x, y in complexity_map:
#         r, g, b = pixels[y, x]
#         binary_data += to_binary(int(r))[-1] + to_binary(int(g))[-1] + to_binary(int(b))[-1]
...
#         """
#         <style>
#         body { background-color: #f0f4f8; font-family: 'Arial', sans-serif; color: #333; }
#         .stButton>button {
#             background-color: #ff6347; color: white; border-radius: 8px;
#             padding: 10px 20px; font-size: 15px; border: none;
#         }
#         .stButton>button:hover { background-color: #e5533d; }
#         .stTextInput>div>input, .stTextArea>div>textarea {
#             background-color: #fafafa; border-radius: 8px; padding: 10px;
#         }
#         h1, h2, h3 { color: #ff6347; }
#         .reportblock { background: #fff3f0; padding: 12px; border-radius: 8px; margin-bottom: 8px; }
#         </style>
#         """,
